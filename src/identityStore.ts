...
function getDfxDefaultPemPath(): string | null {
...
export function pemToIdentity(pem: string) {
  try {
...
export function currentRepdaoPemPath(): string | undefined {
  try {
...
function dfxWhoAmI(): string | undefined {
  try {
...
export function listDfxIdentities(): { names: string[]; current?: string } {
...
export function dfxPemPath(name: string): string {
  return join(DFX_ROOT, name, 'identity.pem'); // dfx stores PEMs here
}

/** List repdao-saved identities. */
export async function list(): Promise<{ names: string[]; current?: string }> {
  ensureStoreDir();
...
export function identityPathOrThrow(): string {
  // 1) explicit repdao current
  const repdao = currentRepdaoPemPath();
  if (repdao) return repdao;
...
export function resolveIdentityPath(opts: { pem?: string }): string {
  if (opts.pem) return resolve(opts.pem);
  ...
  try {
    ...
    if (dfxPem) return dfxPem;

    // no repdao current and no dfx default -> clear message
    throw new Error(
      'No identity set. Use --pem or "repdao id:new|id:import|id:use", ' +
...
export async function use(name: string) {
  ...
  if (!existsSync(p)) throw new Error(`repdao identity "${name}" not found. Use id:import or id:new.`);
  writeFileSync(CURRENT_SYMLINK, p, 'utf8');
}

export async function importPem(name: string, pemFile: string) {
  ensureStoreDir();
...
export async function newIdentity(name: string) {
  ensureStoreDir();
  const dest = join(STORE_DIR, `${name}.pem`);
...
export async function exportPem(name: string, dest?: string) {
  ensureStoreDir();
  const src = join(STORE_DIR, `${name}.pem`);
  if (!existsSync(src)) throw new Error(`repdao identity "${name}" not found.`);
...
export async function del(name: string) {
  ensureStoreDir();
  const p = join(STORE_DIR, `${name}.pem`);
  if (!existsSync(p)) throw new Error(`repdao identity "${name}" not found.`);
